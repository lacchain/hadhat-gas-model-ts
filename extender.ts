...
extendEnvironment((hre: HardhatRuntimeEnvironment) => {
  ...
  hre.lacchain = lazyObject(() => {
    const gasModelProvider = new GasModelProvider( (hre.network.config as HttpNetworkUserConfig).url );

    return {
      provider: gasModelProvider,
      getSigners: () => {
        const { privateKeys, nodeAddress, expiration } = hre.network.config as HttpNetworkUserConfig;
        return (privateKeys||[]).map( (privateKey: string) => new GasModelSigner(privateKey, gasModelProvider, nodeAddress, expiration));
      },
